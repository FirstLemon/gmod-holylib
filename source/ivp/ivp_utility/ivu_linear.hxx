...
public:
    ...
    inline void byte_swap() {
        for (int i = 0; i < 3; ++i) {
            std::uint32_t temp;
            std::memcpy(&temp, &k[i], sizeof(std::uint32_t));
            ivp_byte_swap4(temp);
            std::memcpy(&k[i], &temp, sizeof(std::uint32_t));
        }

#ifdef IVP_VECTOR_UNIT_FLOAT
        {
            std::uint32_t temp;
            std::memcpy(&temp, &hesse_val, sizeof(std::uint32_t));
            ivp_byte_swap4(temp);
            std::memcpy(&hesse_val, &temp, sizeof(std::uint32_t));
        }
#endif
